...
class FuturisticPPGViewer:
    def __init__(self, root):
        ...
        # --- View Control ---
        self.view_mode = "WAVE" # "WAVE" or "FFT"
    ...
    def setup_gui(self):
        ...
        # Switch Button
        self.switch_btn = tk.Button(bottom_bar, text="[ SWITCH VIEW: WAVE ]", 
        ...
        self.switch_btn.pack(side="right", padx=10, pady=2)
    ...
    def toggle_view(self):
        """切換顯示模式：WAVE -> SWEEP -> FFT -> WAVE"""
        self.canvas.delete("all")
        self.line_id = None 

        if self.view_mode == "WAVE":
            self.view_mode = "SWEEP"
            self.switch_btn.config(text="[ VIEW: SWEEP ]", fg=COLORS["NEON_RED"])
            # 掃描模式在 draw_sweep_mode 中直接繪製段落，無需預先建立線物件

        elif self.view_mode == "SWEEP":
            self.view_mode = "FFT"
            self.switch_btn.config(text="[ VIEW: FFT ]", fg=COLORS["NEON_GREEN"])
            
        else: # FFT -> WAVE
            self.view_mode = "WAVE"
            self.switch_btn.config(text="[ VIEW: WAVE ]", fg=COLORS["NEON_CYAN"])
            self.line_id = self.canvas.create_line(0,0,0,0, fill=COLORS["NEON_CYAN"], width=2, smooth=True)
    ...
    def animate_graph(self):
        ...
        if len(self.raw_buffer) >= AVG_WINDOW * 2:
            ...
            if self.view_mode == "WAVE":
                self.draw_wave_mode()
            elif self.view_mode == "SWEEP": # [修改] 呼叫掃描繪圖
        ...
        # 2. Background BPM check (Always runs regardless of view)
    ...
    def glow_line(self, coords, width, color, blur_radius=8):
        """
        Create a glowing line effect using PIL.
        Renders line to image, blurs it, returns as PhotoImage.
        Only affects the line itself, not grid or background.
        """
        if len(coords) < 4:
            return None
        ...
        # Create transparent image with padding for blur overflow
        ...
        # Convert coords to points with padding applied
        points = []
        for i in range(0, len(coords), 2):
            x = coords[i] + padding
            y = coords[i+1] + padding
            points.append((x, y))
        
        # Draw line with higher opacity for brighter glow
        draw.line(points, fill=color_rgb + (255,), width=width)
    ...
    def draw_wave_mode(self):
        ...
        if len(coords) > 4:
             # Delete old glow to redraw fresh
             self.canvas.delete('glow')
             
             # 1. Create glow layer (render to PIL, blur, composite back)
             # Larger blur radius for more visible glow
             glow_photo = self.glow_line(coords, width=6, color=COLORS["NEON_CYAN"], blur_radius=20)
             if glow_photo:
                 # Store reference to prevent garbage collection
                 self.glow_photo = glow_photo
                 # Create glow image on canvas (behind the line)
                 glow_id = self.canvas.create_image(0, 0, image=glow_photo, anchor='nw', tags='glow')
                 # Move glow to back (behind everything)
                 self.canvas.tag_lower('glow')
                 self.canvas.tag_lower(glow_id)
             
             # 2. Draw sharp main line on top
