...
class FuturisticPPGViewer:
    def __init__(self, root):
        ...
        self.ecg_display_buffer = collections.deque([GRAPH_HEIGHT/2] * DATA_POINTS, maxlen=DATA_POINTS) 
        self.ecg_sweep_buffer = [GRAPH_HEIGHT/2] * DATA_POINTS 
    ...
    def draw_sweep_mode(self):
        ...
        for i in range(DATA_POINTS - 1):
            ...
            dim_factor = 1 - (age / DATA_POINTS)**3
    ...
    def draw_ecg_sweep_mode(self):
        # [Fix] Playback logic for SWEEP
        # Calculate how many samples to consume this frame to match 128Hz
        # 128 samples / 1000ms * REFRESH_RATE_MS
        samples_needed = 128.0 * (REFRESH_RATE_MS / 1000.0) # ~2.56
        ...
        new_points = []
        with self.ecg_lock:
            # If queue is getting too big (lag), speed up
            if len(self.ecg_playback_queue) > 1000:
                points_to_pop += 2
        ...
        for val in new_points:
            ...
            self.ecg_sweep_idx = (self.ecg_sweep_idx + 1) % DATA_POINTS
        ...
        for i in range(DATA_POINTS - 1):
            age = (self.ecg_sweep_idx - i) % DATA_POINTS
            dim_factor = 1 - (age / DATA_POINTS)**3
            ...
            x1 = (i + 1) * x_stretch
            y1 = self.ecg_sweep_buffer[(i + 1) % DATA_POINTS]
